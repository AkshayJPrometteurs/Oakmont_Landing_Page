    ...
    return (
        <SectionLayout id={'faqs-section'} headingText={'Frequently Asked Questions'}>
            ...
            <Accordion variant="splitted">
                {faqs.map((data,index) => {
                    return(<AccordionItem className='shadow rounded-md border' key={index} aria-label={data.title} title={data.title}>
                        <p>{data.description}</p>
                    </AccordionItem>)
                })}
            </Accordion>
            <div className='text-center'>
                <Button color="primary" className='mt-6 rounded-lg'>Ask A Question <IoIosArrowForward/></Button>
            </div>
        </SectionLayout>
    )
}
