...
const TwoStepVerification = ({
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      if (data?.status_code === 200 && data?.success) {
        ...
        setResendTime(20);
        setIsApiErrors({
          message: "Security code resend on your email",
          type: "success",
        });
      }
    } catch (error) {
  ...
  return (
    <GuestLayout
      ...
      headerIcon={
        <Image src={MessageEmoji} height={5} width={25} alt="emoji" />
      }
      headerPara={
      ...
      alertVisibility={{
        description: isApiErrors?.message,
        type: isApiErrors?.type,
        visible: isVisible,
      }}
    >
      <Form validationBehavior="native" onSubmit={onSubmit}>
        <h1 className="text-default-500 text-small">
        ...
        <InputOtp
          ...
          classNames={{
            segment: "min-w-6 sm:w-12 h-auto ", 
            segmentWrapper: "w-full gap-x-2 sm:gap-x-4", 
          }}
          variant="bordered"
          name="code"
          size="lg" 
      ...
      <div className="flex justify-center gap-2 items-center mt-4">
        ...
        >
          {!isResendCodeLoader && resendTime > 0
            ? `${resendTime} secs`
            : "Resend"}
        </Button>
      </div>
