...
export default function Header() {
  ...
  const {
    ...
    onOpenChange: onOpenChangeAppDrawer,
  } = useDisclosure();
  const {
    isOpen: isOpenLogOut,
    onOpen: onOpenLogOut,
  ...
  const handleLogOut = async () => {
    ...
    try {
      ...
      if (data?.status_code === 200 && data?.success) {
        ...
        onCloseLogOut();
        toast(<Alert color="success" title={data?.message} />, {
          closeButton: false,
        });
        setTimeout(() => {
          location.reload();
  ...
  return (
    <Navbar
      className="bg-white shadow navbar-heading font-dm-sans overflow-hidden"
      maxWidth="xl"
    >
      <NavbarBrand>
        <Link href="/">
      ...
      <NavbarContent justify="end" className="gap-1 sm:gap-2 md:gap-4">
          ...
          {isAuthenticated ? (
            <Dropdown placement="bottom-center">
              <DropdownTrigger className="cursor-pointer">
                <div className="flex items-center rounded-full px-2 sm:px-3 py-2 border border-gray-400">
              ...
              <DropdownMenu aria-label="Profile Actions" variant="flat">
                <DropdownItem key="profile" className="gap-2">
                  ...
                  />
                  <p className="font-bold text-primaryColor text-center text-base">
                    {user && user.username}
                  </p>
            ...
            <Link
              href="/login"
              className="px-3.5 py-2 md:px-5 md:py-2.5 text-sm md:text-base rounded-full border border-gray-400"
            >
              Login
            </Link>
          )}
        ...
        <NavbarItem>
          <Link
            href={isAuthenticated ? "manage-subscription" : "/signup"}
            className="bg-primaryColor px-3 py-2 md:px-5 md:py-2.5 text-sm md:text-base rounded-full text-white"
          >
            {isAuthenticated ? "Manage Subscription" : "Get Started"}
          </Link>
        </NavbarItem>
      ...
      <DrawerModule
        ...
        onOpenChange={onOpenChangeAppDrawer}
        placement={"left"}
        className="max-w-[15rem!important]"
        header={
          <img alt="logo" src="assets/images/logo.svg" className="h-12" />
        }
        headerClassName={"flex flex-col justify-start gap-1 pl-0"}
      >
        {navItems.map((item) => (
          <Link
            key={item.url}
            color="foreground"
            href={item.url}
            className="text-center hover:bg-primaryColor hover:text-white w-full p-2 rounded-md"
          >
            {item.name}
          </Link>
        ))}
      ...
      <Modals
        ...
        modalBodyClass={"p-0 gap-[0!important]"}
        isDismissable={false}
      >
        ...
        <div
          className={`bg-gray-100 flex items-center gap-4 justify-center py-4 px-2 ${inter.className}`}
        >
          {logOutLoader ? (
            <Button
              color="primary"
            ...
            <Fragment>
              <Button color="default" className="px-6" onPress={onCloseLogOut}>
                No! Cancel
              </Button>
              <Button color="danger" className="px-6" onPress={handleLogOut}>
                Yes! Logout
              </Button>
            </Fragment>
          )}
